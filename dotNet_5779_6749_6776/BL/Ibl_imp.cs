...
namespace BL
{
    public class Ibl_imp : IBL
    {
        ...
        {
            HostingUnit hu = DAL.DalFactory.getDal().GetHostingUnit(thisOrder.HostingUnitKey);
            GuestRequest gr = DAL.DalFactory.getDal().GetGuestRequest(thisOrder.GuestRequestKey);
        ...
        public List<Order> AllExpiredOrders(int numDays)
        {
            ...
            try
            {
                List<Order> newList = (from Order item in AllOrders()
                                       ...
                                       select item.Clone()).ToList();
                return newList.Count == 0 ? throw new MyException("There is not expired orders!") : newList;
            }
            catch (MyException ex)
            {
                throw new MyException(ex.Message);
